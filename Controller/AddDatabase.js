...
async function addinthedatabase(Semester,Group,Day,StartTime,EndTime,RoomId,TeacherId,CourseCode,CourseType){
...
//     console.log(CourseType)
    ...
    return new Promise(async (resolve,reject)=>{
        await connection.then(async (conn)=>{
            var que = "INSERT INTO `MasterCseTable`(`Semester`, `Group_`, `Day`, `StartTime`, `EndTime`, `RoomId`, `TeacherId`, `CourseCode`,`CourseType`) VALUES (?,?,?,?,?,?,?,?,?)"
            conn.query(que,[Semester,Group,Day,StartTime,EndTime,RoomId,TeacherId,CourseCode,CourseType]).then(([row,field])=>{
